<?PHP
	...
	foreach($json->data as $key => $json_arr) {
		...
		//PK 0인경우 추가
		if ($json_arr->pk == 0) {
			$id = $db->addCareer($_SESSION['ID'], $start_date, $end_date, $json_arr->text, $json_arr->subText);
			$id_arr[] = $id;
		} else {
			$result = $db->setCareer($_SESSION['ID'], $json_arr->pk, $start_date, $end_date, $json_arr->text, $json_arr->subText);
			$id_arr[] = $json_arr->pk;
		}
	}
	$id_arr = implode(',', $id_arr);
	$db->deleteCareer($_SESSION['ID'], $id_arr);
	
	$careerArr = $db->getCareer($_SESSION['ID']);
	if ($careerArr) {
		$arr = array(Consts::RESULT => Consts::SUCCESS, "workInfo" => $careerArr);	
	} else {
